...
2. **사업자등록 이야기를 여기서 절대 꺼내지 않는다.**
   ...
   실결제 전환은 `/ship`과 `docs/02-실결제-전환-체크리스트.md`가 다룬다.
   사용자가 먼저 물어보면 "지금은 신경 안 쓰셔도 돼요. 다 만들고 나서 `/ship`에서 알려드릴게요"라고만 답한다.
...
> 다만 웹훅과 API 로그가 없어서 `/ship`에서 다시 해야 한다. 그러니 지금 가입하는 편이 낫다고 말해준다.

### 5단계. Vercel (배포)

[vercel.com](https://vercel.com) → GitHub로 로그인. 지금은 여기까지만 한다.
실제 배포는 `/ship`에서 한다. 키를 미리 받을 필요 없다.
