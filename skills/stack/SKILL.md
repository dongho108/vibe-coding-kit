...
| 폴더 구조 | 결제 연동 (`/toss-payments`) |
| Supabase 연결과 스키마 | 배포 (`/ship`) |
| 구글 로그인 | 디자인 (`/design`) |
...
이미 폴더에 `.env.local` 이 있으면 덮어쓰지 않게 주의한다. `/setup` 에서 만든 것이다.
...
`toss_payment_key` 같은 결제 컬럼을 지금 미리 넣어둔다. `/toss-payments` 가 이 자리를 쓴다.
...
`/setup` 에서 Supabase 대시보드에 Google Client ID/Secret을 이미 넣어뒀다. 코드만 붙인다.
